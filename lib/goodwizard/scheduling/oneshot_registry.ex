defmodule Goodwizard.Scheduling.OneShotRegistry do
  @moduledoc """
  In-memory registry mapping `job_id -> timer_ref` for one-shot tasks.

  Tracks `:timer` references so one-shot jobs can be cancelled before
  ...
  @doc """
  Cancel a one-shot job by job_id.
