defmodule Goodwizard.Actions.Scheduling.ScheduledTask do
  @moduledoc """
  ...
  The scheduler pid is tracked in `ScheduledTaskRegistry` for later cancellation.
  ...
  **external_id** (e.g. a Telegram chat_id) rather than a Messaging room
  ...
  defp warn_high_frequency(schedule) do
    if schedule in @high_frequency_patterns do
      Logger.warning(
        "Scheduled task schedule #{inspect(schedule)} runs every minute — consider a less frequent interval"
      )
    end
  end

  defp check_job_limit do
    max_jobs = Goodwizard.Config.get(["scheduled_tasks", "max_jobs"]) || @default_max_jobs

    case ScheduledTaskStore.list() do
      {:ok, jobs} when length(jobs) >= max_jobs ->
        {:error,
         "Scheduled task limit reached (#{max_jobs}). Cancel existing tasks before scheduling new ones."}
  ...
  defp validate_cron(expr) do
    case CronExpression.Parser.parse(expr) do
      {:ok, _} -> :ok
      {:error, reason} -> {:error, "Invalid schedule expression: #{expr} — #{inspect(reason)}"}
    end
  end
  ...
  defp register_and_persist(agent_id, signal, job) do
    ...
    case Jido.Scheduler.run_every(tick_fn, job.schedule, []) do
      {:ok, sched_pid} ->
        ...
        # runner outlive the caller. ScheduledTaskRegistry monitors it independently.
        Process.unlink(sched_pid)
        ScheduledTaskRegistry.register(job.job_id, sched_pid)
      ...
      {:error, reason} ->
        {:error, "Failed to register scheduled task: #{inspect(reason)}"}
    end
  end
  ...
  defp dispatch_tick(agent_id, signal) do
    case Goodwizard.Jido.whereis(agent_id) do
      ...
      nil -> Logger.warning("Scheduled task tick: agent #{agent_id} not found, skipping")
    end
  end
