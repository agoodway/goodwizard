defmodule Goodwizard.Actions.Scheduling.OneTimeTest do
  use ExUnit.Case, async: false

  alias Goodwizard.Actions.Scheduling.OneTime
  alias Goodwizard.Scheduling.OneTimeRegistry

  @test_workspace Path.join(
                    System.tmp_dir!(),
                    "one_time_action_test_#{System.unique_integer([:positive])}"
                  )
  ...
  describe "delay mode" do
    ...
    test "zero delay_minutes returns error" do
      ...
      assert {:error, msg} = OneTime.run(params, %{})
    ...
    test "delay_minutes at max boundary succeeds" do
      ...
      assert {:ok, _result} = OneTime.run(params, %{})
    end
  ...
  describe "job ID" do
    test "uses one_time_ prefix with 16 hex chars" do
      params = %{delay_minutes: 5, task: "test", channel: "cli", external_id: "direct"}
      assert {:ok, result} = OneTime.run(params, %{})
      assert to_string(result.job_id) =~ ~r/^one_time_[0-9a-f]{16}$/
    end

    test "same fires_at/task/channel/external_id produce same job_id" do
      fires_at = ~U[2026-06-15 09:00:00Z]
      id1 = OneTime.generate_job_id(fires_at, "check", "cli", "direct")
      id2 = OneTime.generate_job_id(fires_at, "check", "cli", "direct")
      assert id1 == id2
    end
    ...
    test "different inputs produce different job_ids" do
      fires_at = ~U[2026-06-15 09:00:00Z]
      id1 = OneTime.generate_job_id(fires_at, "check", "cli", "direct")
      id2 = OneTime.generate_job_id(fires_at, "other", "cli", "direct")
      assert id1 != id2
    end
  end

  describe "persistence" do
    test "saves job to OneTimeStore after scheduling", %{one_time_dir: one_time_dir} do
      params = %{delay_minutes: 30, task: "persist test", channel: "cli", external_id: "direct"}
      assert {:ok, result} = OneTime.run(params, %{})

      path = Path.join(one_time_dir, "#{result.job_id}.json")
    ...
    test "registers timer in OneTimeRegistry after scheduling" do
      params = %{delay_minutes: 30, task: "registry test", channel: "cli", external_id: "direct"}
      assert {:ok, result} = OneTime.run(params, %{})

      assert {:ok, _tref} = OneTimeRegistry.lookup(result.job_id)

      # Cleanup
      OneTimeRegistry.cancel(result.job_id)
    end

    test "deliver/3 cleans up persisted file and registry entry", %{one_time_dir: one_time_dir} do
      params = %{delay_minutes: 30, task: "cleanup test", channel: "cli", external_id: "direct"}
      assert {:ok, result} = OneTime.run(params, %{})

      job_id = result.job_id
      path = Path.join(one_time_dir, "#{job_id}.json")
      assert File.exists?(path)
      assert {:ok, _} = OneTimeRegistry.lookup(job_id)

      # Simulate delivery (agent won't be found but cleanup should still happen)
      signal = %{type: "test"}
      OneTime.deliver("nonexistent_agent", signal, job_id)
