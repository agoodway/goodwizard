defmodule Goodwizard.Actions.Scheduling.ScheduledTaskRunnerTest do
  ...
  @test_workspace Path.join(
                    System.tmp_dir!(),
                    "scheduled_task_runner_test_#{System.unique_integer([:positive])}"
                  )
  ...
  describe "run_isolated/3" do
    test "returns :at_capacity when max_concurrent_agents is 0" do
      ...
      Goodwizard.Config.put(["scheduled_tasks", "max_concurrent_agents"], 0)

      on_exit(fn ->
        Goodwizard.Config.put(["scheduled_tasks", "max_concurrent_agents"], nil)
      end)

      assert {:error, :at_capacity} = ScheduledTaskRunner.run_isolated("test task", "room_1")
    end
    ...
    test "accepts keyword opts for model override" do
      ...
      assert {:error, :at_capacity} =
               ScheduledTaskRunner.run_isolated("test task", "room_1",
