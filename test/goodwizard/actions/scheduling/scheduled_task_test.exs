defmodule Goodwizard.Actions.Scheduling.ScheduledTaskTest do
  ...
  @test_workspace Path.join(
                    System.tmp_dir!(),
                    "scheduled_task_test_#{System.unique_integer([:positive])}"
                  )
  ...
  describe "run/2" do
    ...
    test "auto-resolves channel from telegram agent context" do
      ...
      assert {:ok, result} = ScheduledTask.run(params, context)
      assert result.channel == "telegram"
    ...
    test "rejects channel without external_id" do
      ...
      assert {:error, msg} = ScheduledTask.run(params, %{})
    ...
    test "cron alias @daily works" do
      ...
      assert {:ok, _result} = ScheduledTask.run(params, %{})
    end
    ...
    test "invalid schedule expression returns error" do
      params = %{schedule: "not-a-cron", task: "test", channel: "cli", external_id: "direct"}
      assert {:error, msg} = ScheduledTask.run(params, %{})
      assert msg =~ "Invalid schedule expression: not-a-cron"
    end

    test "empty schedule expression returns error" do
      params = %{schedule: "", task: "test", channel: "cli", external_id: "direct"}
      assert {:error, msg} = ScheduledTask.run(params, %{})
      assert msg =~ "Invalid schedule expression"
    end

    test "same params produce same job_id" do
      params = %{schedule: "0 9 * * *", task: "check", channel: "cli", external_id: "direct"}
      assert {:ok, r1} = ScheduledTask.run(params, %{})
      assert {:ok, r2} = ScheduledTask.run(params, %{})
    ...
    test "different params produce different job_ids" do
      ...
      assert {:ok, r1} = ScheduledTask.run(params1, %{})
      assert {:ok, r2} = ScheduledTask.run(params2, %{})
      assert r1.job_id != r2.job_id
    end
  end
  ...
  describe "mode affects job_id" do
    test "same task in different modes gets distinct job IDs" do
      ...
      assert {:ok, r_main} = ScheduledTask.run(Map.put(base, :mode, "main"), %{})
      assert {:ok, r_isolated} = ScheduledTask.run(Map.put(base, :mode, "isolated"), %{})
