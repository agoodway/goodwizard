defmodule Mix.Tasks.Goodwizard.SetupTest do
  ...
  describe "run/1" do
    ...
    test "creates knowledge base directory structure and seeds schemas" do
      with_tmp_dir(fn tmp ->
        Setup.run(["--base-dir", tmp])

        knowledge_base_dir = Path.join(tmp, "knowledge_base")
        assert File.dir?(knowledge_base_dir), "Expected knowledge_base/ directory to exist"

        schemas_dir = Path.join(knowledge_base_dir, "schemas")
        assert File.dir?(schemas_dir), "Expected knowledge_base/schemas/ directory to exist"
        ...
        for type <- @entity_types do
          ...
          type_dir = Path.join(knowledge_base_dir, type)
          assert File.dir?(type_dir), "Expected knowledge_base/#{type}/ directory to exist"
        end
      end)
    end
    ...
    test "is idempotent — running twice does not raise" do
      with_tmp_dir(fn tmp ->
        ...
        for type <- @entity_types do
          assert File.exists?(Path.join([tmp, "knowledge_base", "schemas", "#{type}.json"]))
        end
      end)
    end
    ...
    test "raises when Brain.ensure_initialized fails" do
      with_tmp_dir(fn tmp ->
        # Create the knowledge_base dir as a regular file to make mkdir_p inside ensure_initialized fail
        knowledge_base_dir = Path.join(tmp, "knowledge_base")
        File.write!(knowledge_base_dir, "not a directory")

        assert_raise Mix.Error, ~r/could not initialize knowledge base/, fn ->
          Setup.run(["--base-dir", tmp])
        end
      end)
    end
