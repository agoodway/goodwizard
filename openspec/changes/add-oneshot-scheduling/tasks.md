...
- [x] 1.1 Create `lib/goodwizard/actions/scheduling/one_shot.ex` with `use Jido.Action`, name `schedule_oneshot_task`, and schema defining `delay_minutes` (optional positive integer), `at` (optional string), `task` (required string), and `room_id` (required string)
...
- [x] 1.5 Derive deterministic job ID using `:"oneshot_#{:erlang.phash2({task, room_id, fire_at})}"`

## 2. Signal Compatibility

- [x] 2.1 Build CronTick-compatible message payload `%{type: "cron.task", task: task, room_id: room_id}` in the action's `run/2`
...
- [x] 3.1 Add `Goodwizard.Actions.Scheduling.OneShot` to the tools list in `Goodwizard.Agent`, adjacent to `Goodwizard.Actions.Scheduling.Cron`
...
- [x] 4.6 Test message payload matches CronTick format `%{type: "cron.task", task: _, room_id: _}`
