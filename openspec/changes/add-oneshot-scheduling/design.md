...
- Recurring schedules (already handled by the existing Cron action)
...
The one-shot message payload uses the same `%{type: "cron.task", task: task, room_id: room_id}` shape as the recurring cron action. This means the existing signal handling pipeline — which routes `cron.task` messages to the appropriate room — works without modification.

**Alternative considered**: A new `%{type: "oneshot.task", ...}` message type. Rejected because it would require adding a new signal handler branch with identical behavior. The "cron" naming is slightly misleading for a one-shot, but the payload shape is what matters for routing, not the type string.
...
A deterministic job ID is computed as `:"oneshot_#{:erlang.phash2({task, room_id, scheduled_at})}"` where `scheduled_at` is the computed fire time. This provides a stable identifier for logging and potential future cancellation support, while avoiding collisions with cron job IDs (which use the `cron_` prefix).

## Risks / Trade-offs

- **In-memory only**: Like the current cron action, one-shot schedules are backed by `Process.send_after` and lost on restart. The `persist-cron-jobs` change can be extended to cover one-shots in the future.
...
- **No cancellation**: Unlike cron (which has `CronCancel`), there is no `ScheduleCancel` in Jido for `Process.send_after` timers. Once a one-shot is scheduled, it will fire. Cancellation support would require storing the timer reference, which is out of scope for this change.
