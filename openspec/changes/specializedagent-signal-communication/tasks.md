...
- [ ] 1.1 Define signal type constants in a `Goodwizard.Subagent.Signals` module: `@progress "goodwizard.specializedagent.progress"`, `@result "goodwizard.specializedagent.result"`, `@error "goodwizard.specializedagent.error"`
...
- [ ] 2.1 Create `lib/goodwizard/plugins/specializedagent_router.ex` using `Jido.Plugin` with signal patterns matching `"goodwizard.specializedagent.*"` and `"jido.agent.child.*"`
- [ ] 2.2 Handle `goodwizard.specializedagent.progress` — write to `Cache.put("specializedagent:progress:#{agent_id}", data)`
- [ ] 2.3 Handle `goodwizard.specializedagent.result` — write to `Cache.put("specializedagent:result:#{agent_id}", data)`, delete progress entry
- [ ] 2.4 Handle `goodwizard.specializedagent.error` — write to `Cache.put("specializedagent:result:#{agent_id}", %{error: reason})`, delete progress entry
...
- [ ] 3.1 Create `lib/goodwizard/actions/specializedagent/emit_progress.ex` with schema `%{message: string}`
...
- [ ] 4.3 Implement sync mode — after emitting directives, poll `Cache.get("specializedagent:result:#{agent_id}")` with 500ms interval until result appears or timeout
- [ ] 4.4 Implement async mode — emit directives and return `{:ok, %{agent_id: id, role: role, status: :running}}` immediately
- [ ] 4.5 Preserve cleanup — on sync timeout or error, emit `Directive.StopChild` to terminate the specializedagent
...
- [ ] 5.2 On ReAct completion, emit a `goodwizard.specializedagent.result` signal to parent with the final result
- [ ] 5.3 On ReAct error, emit a `goodwizard.specializedagent.error` signal to parent with the error reason
