...
- **WHEN** a user schedules a one-time task with `delay_minutes: 20`, `task: "Send daily report"`, and `room_id: "cli:heartbeat"`
- **THEN** the system emits a `Directive.Schedule` with `delay_ms: 1_200_000` and message `%{type: "scheduled_task.task", task: "Send daily report", room_id: "cli:heartbeat"}`

#### Scenario: Zero delay is rejected

- **WHEN** a user schedules a one-time task with `delay_minutes: 0`
- **THEN** the system returns an error indicating the delay must be a positive integer

#### Scenario: Negative delay is rejected

- **WHEN** a user schedules a one-time task with `delay_minutes: -5`
- **THEN** the system returns an error indicating the delay must be a positive integer
...
- **WHEN** a user schedules a one-time task with `at` set to an ISO 8601 datetime 30 minutes in the future, `task: "Send weekly summary"`, and `room_id: "telegram:main"`
...
- **WHEN** a user schedules a one-time task with `at` set to an ISO 8601 datetime that has already passed
...
- **WHEN** a user schedules a one-time task with `at: "not-a-date"`
...
The one-time task message payload SHALL use the format `%{type: "scheduled_task.task", task: task, room_id: room_id}`, identical to the recurring scheduled task action's payload. This ensures the existing signal handling pipeline processes one-time tasks without modification.

#### Scenario: One-time task fires and is processed by signal pipeline

- **WHEN** a one-time task timer fires and delivers its message to the agent
- **THEN** the signal pipeline processes it identically to a recurring scheduled task, routing the task text to the specified room
...
- **WHEN** a user schedules a one-time task with both `delay_minutes: 10` and `at: "2026-02-15T15:00:00Z"`
...
- **WHEN** a user schedules a one-time task with only `task` and `room_id` but no `delay_minutes` or `at`
- **THEN** the system returns an error indicating that exactly one of `delay_minutes` or `at` must be provided

### Requirement: Action registered in agent tools list

The `Goodwizard.Actions.Scheduling.OneTime` action SHALL be registered in the `Goodwizard.Agent` tools list alongside the existing `Goodwizard.Actions.Scheduling.ScheduledTask` action.

#### Scenario: Agent starts with one-time task action available

- **WHEN** the Goodwizard agent starts
- **THEN** the `schedule_one_time_task` tool is available in the agent's tool list and can be invoked by the LLM
