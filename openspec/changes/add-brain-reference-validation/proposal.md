...
Entity references in the brain (e.g. `tasks/abcd1234`, `people/xyz98765`) are never validated. When an entity is deleted, any other entities referencing it retain stale IDs. This leads to silent data corruption — the agent presents references that point to nothing, and users have no way to know their data is inconsistent.
